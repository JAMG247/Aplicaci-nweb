...
<div class="mb-4 flex items-center space-x-2">
    ...
    <form id="searchForm" method="GET" action="{{ route('pacientes.index') }}" 
    ...
    <tbody class="divide-y divide-gray-200">
      @foreach($pacientes as $paciente)
         <tr>
          ...
          <td class="px-6 py-4 whitespace-nowrap">{{ $paciente->rut }}</td>
          ...
          <td class="px-6 py-4 whitespace-nowrap">{{ $paciente->fecha_nacimiento->age }} años</td>
          <td class="px-6 py-4 whitespace-nowrap">{{ $paciente->created_at->format('d/m/Y H:i') }}</td>
          <td class="px-6 py-4 whitespace-nowrap">
            <a href="{{ route('pacientes.show', $paciente->id) }}" class="text-blue-600 hover:text-blue-900">Ver</a>
            <a href="{{ route('pacientes.edit', $paciente->id) }}" class="text-yellow-600 hover:text-yellow-900 ml-2">Editar</a>
            <form action="{{ route('pacientes.destroy', $paciente->id) }}" method="POST" class="inline ml-2">
              @csrf
              @method('DELETE')
